...
export default async function AdminBlogPostsList() {
  ...
  const canCreate = hasPermission(perms, PERMISSIONS.CREATE_BLOG_POSTS);
  const canDelete = hasPermission(perms, PERMISSIONS.DELETE_BLOG_POSTS);
  const canUpdate = hasPermission(perms, PERMISSIONS.UPDATE_BLOG_POSTS);
                  ...
                  {(canUpdate || canDelete) && (
                    <td className="px-6 py-4 text-right">
                      <div className="inline-flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        {canDelete && <DeleteBlogPostButton postId={String(post._id)} compact />}
                      </div>
                    </td>
                  )}
                </tr>
